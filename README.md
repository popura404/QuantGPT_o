...
make setup      # creates venv, installs deps, generates local .env with auth disabled
bash restart.sh # builds frontend and starts server at http://localhost:8003
```

`.env.example` keeps `AUTH_DISABLED=false` and `QUANTGPT_ALLOW_GUEST_BACKTEST=false` for safe deployments.
`make setup` creates a local single-user `.env` with `AUTH_DISABLED=true` so the quick start works without
generating credentials. Production must keep authentication enabled, use a strong `JWT_SECRET_KEY`, set a
strong `QUANTGPT_ADMIN_PASSWORD`, and protect any HTTP MCP exposure with `QUANTGPT_MCP_HTTP_TOKEN`.
...
For Docker, server, reverse-proxy, backup, and rollback procedures, use the [Deployment Guide](docs/DEPLOYMENT.md).
...
curl -X POST http://localhost:8003/api/v1/auto_backtest \
  ...
  -d '{"prompt": "rank(close / ts_mean(close, 20))", "universe": "hs300"}'
```

When using the local `.env` generated by `make setup` (`AUTH_DISABLED=true`), omit the Authorization header.
...
(Get-Content .env) -replace '^AUTH_DISABLED=false','AUTH_DISABLED=true' | Set-Content .env

# 4. 构建前端（需要 Node.js，从 nodejs.org 下载 LTS 版本）
...
alembic -c alembic.deploy.ini upgrade head
```
